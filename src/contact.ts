...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const downloadBtn = document.getElementById("download-portfolio") as HTMLButtonElement | null;
      ...
      try {
        if (sec.imagesOnly) {
          ...
          for (const img of images) {
            ...
            const canvas = await html2canvas(img as any, { scale: 2, useCORS: true, allowTaint: true } as any);
          ...
          const canvas = await html2canvas(el as any, { scale: 2, useCORS: true, allowTaint: true, scrollY: -window.scrollY } as any);
          const imgData = canvas.toDataURL("image/png");
